...
"""
CAM type: dict[key] = cam_map, where key is the validate class
"""
...
def _work(process_id, infer_dataset, args):
    ...
    for iter, pack in enumerate(tqdm(infer_data_loader, position=process_id, desc=f'[PID{process_id}]')):
        ...
        # keys = np.pad(cam_dict['keys'] + 1, (1, 0), mode='constant') # modified
        keys = []
        cams = []
        ...
        keys = np.pad(np.array(keys) + 1, (1, 0), mode='constant') # modified
        if keys.shape[0] == 1:
